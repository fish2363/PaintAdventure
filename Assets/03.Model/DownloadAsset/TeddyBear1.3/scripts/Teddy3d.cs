...
public class Teddy3d : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (!dead)
        {
            ...
            //TRANSLATE
            v3Velocity = rigid.linearVelocity;
            ...
            {
                if (walk == true) rigid.linearVelocity = forcedir * -1f;
                if (walk == false) rigid.linearVelocity = forcedir * -4f;
            }
            if (Input.GetKey(KeyCode.S) && gr == true && speed <= 1f && speed > -1.1f && active == true)
            {
                if (walk == true) rigid.linearVelocity = forcedir * 1f;
                if (walk == false) rigid.linearVelocity = forcedir * 3f;
            }
            ...
            if (speed > 11)
            {
                ...
                foreach (Transform child in allChildren)
                {
                    if (child.GetComponent<Rigidbody>() != null)
                    {
                        ...
                        child.GetComponent<Rigidbody>().linearVelocity = v3VelocityAUX;
                    }
    ...
    IEnumerator StandUp()
    {
        ...
        Pelvis.transform.GetComponent<Rigidbody>().linearVelocity = Vector3.zero;
        anim.SetBool("walk",false);
        anim.SetFloat("run", 0f);
        speed = 0f;
